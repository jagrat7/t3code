  ...
  const runDevinJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const promptResult = yield* Effect.gen(function* () {
        yield* runtime.start();
        // Text generation must never mutate the workspace: force Devin's
        // read-only `ask` mode before configuring or prompting, and fail
        // honestly when the CLI does not advertise it.
        const modeState = yield* runtime.getModeState;
        if (!modeState?.availableModes.some((mode) => mode.id === "ask")) {
          return yield* new TextGenerationError({
            operation,
            detail: "Devin does not advertise a read-only session mode.",
          });
        }

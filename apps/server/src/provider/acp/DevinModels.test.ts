...
it("keeps Fusion pairings for an unavailable lead or sidekick as exact models", () => {
  const fusionOnly = {
    families: [
      {
        slug: "fusion",
        family_label: "Fusion",
        variants: [{ model_uid: "pair-orphan", label: "Fusion (Opus High + SWE High)" }],
      },
    ],
  };
  const models = devinModels(fusionOnly);
  expect(models.map((model) => [model.slug, model.name])).toEqual([
    ["pair-orphan", "Fusion (Opus High + SWE High)"],
  ]);
  expect(models[0]?.fusion).toBeUndefined();
  expect(resolveDevinModel(fusionOnly, { model: "pair-orphan" })).toBe("pair-orphan");
});

it("resolves family aliases to the current account's exact IDs", () => {
  const aliased = {
    families: [{ ...lead, slug: "opus", aliases: ["claude-opus"] }],
  };
  expect(resolveDevinModel(aliased, { model: "claude-opus" })).toBe("high");
});

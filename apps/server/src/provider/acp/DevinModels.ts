...
export function devinModels(catalog: Catalog): ServerProviderModel[] {
  return selectableFamilies(catalog).flatMap((family): ServerProviderModel[] => {
    ...
    // `adaptive` is Devin's own default selection, so it carries the default marker.
    const isDefault =
      family.slug.toLowerCase() === "adaptive" ||
      family.variants.some((variant) => variant.model_uid.toLowerCase() === "adaptive");
    if (variants.length !== family.variants.length) {
      return family.variants.map((variant) => ({
        slug: variant.model_uid,
        name: variant.label,
        isCustom: false,
        ...(variant.model_uid.toLowerCase() === "adaptive" ? { isDefault: true } : {}),
